...
- **Decode thumbnails to card size (dynamic).** Library thumbs are ~1024-1388px shown in ~280-360px cards → GPU sampled a 1MP+ bitmap per card per frame. `BoundedRamImageLoader` now `Bitmap.DecodeToWidth`s local files + the remote disk-cache reload to a **dynamic** width = card pixel width (logical × `RenderScaling`) × 1.15, 64px-bucketed, pushed from `UpdateCardThumbnailHeight`; a size/scale change clears the RAM LRU so thumbs re-decode at the new size. **No quality loss** (decode width ≥ display px at any size/monitor). **Medium scroll ws 2.2→1.5GB, 1%-low 20→25-33, gen0 +146→+4.** The enlarged **preview modal** would be soft at this size, so it binds a separate per-card `PreviewBitmap` decoded at **native** res (loaded on preview, disposed on close via `OnPreviewCardChanged`), layered over the grid-size placeholder.
