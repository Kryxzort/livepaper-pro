...
- **GIF activation = all realized cards + settle-debounced** (Browse grid): `MainWindow.ReconcileBrowseGifs` animates every realized gif card. Browse `ItemsRepeater` uses `VerticalCacheLength=0`, so "realized" ≈ "visible"; `ElementClearing` stops a card's gif as it scrolls out. A per-element `TranslatePoint` viewport test was tried and **removed** — it resolved to 0 visible cards (timing/coord-space), so no gif ever auto-started. `ScheduleGifReconcile` (160ms) suppresses decode during a fast scroll so a fling doesn't decode a full-res preview per card passed. Do **NOT** cap with a process-wide active-gif counter — it leaks when card VMs are discarded (placeholder→real swap, reloads) while `IsGifActive=true`.
- **Pause grid gifs while the preview modal is open.** Two animation loops over the *same* gif at once corrupt each other (patchwork of different frames) — the library's frame decode isn't safe for concurrent playback of one gif, even across independent `FrameRenderer`s. On `PreviewCard != null` the grid cards call `DeactivateGifKeepSource()` (stops the grid `Image` via `IsGifActive=false` but, unlike the normal setter, does **not** `ReleaseGifSource()` — the modal binds the same card's `GifSource` and keeps playing solo); on close, `ReconcileBrowseGifs` re-activates. Wire this in `OnViewModelPropertyChanged` for `PreviewCard` — **brace the open-branch foreach** or the `else` (close) dangles onto the inner `if` and resume never fires.
